...
<?php include 'add_button/add_course.php'; ?>
    ...
    <?php
        if(isset($_GET['search_name'])){
          ...
          $result="";
            ...
            <?php
            $get=mysql_query("SELECT * FROM course_tbl INNER JOIN department_tbl USING(d_id) WHERE course LIKE '%$result%' ORDER BY course");
            while($query = mysql_fetch_array($get)){
                echo '<tr class="active">';
                echo '<td>'.$query['course'].'</td>';
                echo '<td>'.$query['department'].'</td>';
                echo '<td><a href="course_edit.php?c_id='.$query['c_id'].'" class = "btn btn-info btn-xs"><i class="fa fa-fw fa-edit"></i>Edit</a> &nbsp; '; 
             

              echo '<a href="del/course_del.php
